...
public class TypeWriter1_2 : MonoBehaviour
{
    //변경할 변수
	public float delay;
    ...
    public GameObject all_UI;       //우측 상단 UI들     
    /*---------------------------------------------------------------------*/
    //시작과 동시에 타이핑시작
    void Start()
    {
        Get_Typing(dialog_cnt,fulltext);
        count = fulltext.Length;    //나와야 할 총 대사 가져옴 (-1 안한건 대사 끝나고 클릭하면 다음으로 동작으로 넘어가게 하려고)
    }
    ...
    void Update()
    {
        ...
        {
            End_Typing();
            if(cnt == count) //대화 다 끝났으면 대사창 끄고 오른쪽 위 UI On
            {
